...
string rootPath = @"C:\Users\ZhouJie\source\repos\Sdcb.PictureSpeaks";
...
public class ChatgptService
{
	...
	public async Task<Dictionary<string, int>> QueryGPT(string[] words, CancellationToken ct = default)
	{
		...
		ChatMessage[] messages = [systemPrompt, new ChatMessage(ChatRole.User, string.Join(",", words))];
		...
		while (true)
		{
			try
			{
				...
				Dictionary<string, int> result = JsonSerializer.Deserialize<Dictionary<string, int>>(resp.Value.Choices[0].Message.Content)!;
